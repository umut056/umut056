...
import { DEMO_ACCOUNTS, TEST_LOGIN_BY_KEY } from "./lib/demoAccounts.js";
import {
  authenticateUser,
...
const SEED_USERS = [
  ...
  { id:"coach1", role:"coach", name:"Test Koç", email:TEST_LOGIN_BY_KEY.coach.email, password:TEST_LOGIN_BY_KEY.coach.password, refCode:"TESTAPK", phone:"+90 555 111 22 33", createdAt:"2026-06-03", status:"active", clients:["client1","client2","client3"], clientMessagesOpen:true },
  { id:"client1", role:"client", name:"Test Danışan", email:TEST_LOGIN_BY_KEY.client1.email, password:TEST_LOGIN_BY_KEY.client1.password, coachId:"coach1", coachRef:"TESTAPK", phone:"+90 555 234 10 20", program:"Program atanmadı", goal:"Program atama testi", createdAt:"2026-06-03", createdAtTime:"2026-06-03T09:00:00.000Z", startedAt:"2026-06-03T09:00:00.000Z", programStartDate:"2026-06-03", programEndDate:"2026-07-03", status:"active", compliance:0, tasks:[], pendingToday:0, missedToday:0, photoPendingToday:0, weeklyAverage:0, streakDays:4, body:{height:170,age:30,gender:"female",start:80,current:80,target:70,water:50,fat:30,muscle:34,bmi:27.7,waist:88,hip:103,chest:94,ideal:"62-68 kg"} },
  { id:"client2", role:"client", name:"Elif Yılmaz", email:TEST_LOGIN_BY_KEY.client2.email, password:TEST_LOGIN_BY_KEY.client2.password, coachId:"coach1", coachRef:"TESTAPK", phone:"+90 555 333 44 55", program:"4 Temel Program", programTemplateId:"four-foundation", goal:"Kilo kontrolü ve enerji takibi", createdAt:"2026-06-03", createdAtTime:"2026-06-03T09:00:00.000Z", startedAt:"2026-06-03T09:00:00.000Z", programStartDate:"2026-06-03", programEndDate:"2026-07-03", status:"active", compliance:80, tasks:[true,true,false,false,false,false,false], pendingToday:5, missedToday:0, photoPendingToday:5, weeklyAverage:82, streakDays:5, body:{height:168,age:31,gender:"female",start:72,current:67.5,target:63,water:52.4,fat:27.8,muscle:35.1,bmi:24.2,waist:78,hip:99,chest:91,ideal:"58-64 kg"} },
  { id:"client3", role:"client", name:"Mert Demir", email:TEST_LOGIN_BY_KEY.client3.email, password:TEST_LOGIN_BY_KEY.client3.password, coachId:"coach1", coachRef:"TESTAPK", phone:"+90 555 444 55 66", program:"Kilo Alma Programı", programTemplateId:"weight-gain", goal:"Sağlıklı kilo alma ve öğün düzeni", createdAt:"2026-06-03", createdAtTime:"2026-06-03T09:00:00.000Z", startedAt:"2026-06-03T09:00:00.000Z", programStartDate:"2026-06-03", programEndDate:"2026-07-03", status:"active", compliance:65, tasks:[true,false,false,false,false,false], pendingToday:5, missedToday:1, photoPendingToday:5, weeklyAverage:68, streakDays:2, body:{height:181,age:36,gender:"male",start:64,current:65.8,target:72,water:49.1,fat:22.4,muscle:38.8,bmi:20.1,waist:78,hip:94,chest:96,ideal:"76-82 kg"} },
];
const SEED_MSGS = [
...
const CoachMsgs=({user,allUsers})=>{
  ...
  const unreadHeadline=unreadConversationHeadline(unreadSummaries,unreadTotal);
  const unreadDetail=unreadConversationDetail(unreadSummaries);
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"100%",background:C.mist,position:"relative"}}>
      ...
      <div style={{flex:1,overflowY:"auto"}}>
        {unreadTotal>0&&<div style={{padding:"12px 20px 4px"}}><Card onClick={()=>{const top=clients.find(c=>c.id===unreadSummaries[0]?.senderId);if(top)setSel(top);}} style={{padding:"14px",border:`1px solid ${C.mint}`,background:"linear-gradient(135deg,#f0fff8,#ffffff)",cursor:"pointer",boxShadow:"0 12px 28px rgba(13,61,43,.08)"}}><div style={{display:"flex",alignItems:"center",gap:12}}><div style={{width:42,height:42,borderRadius:15,background:C.emerald,display:"flex",alignItems:"center",justifyContent:"center",color:C.white,fontWeight:900,...F}}>{unreadTotal}</div><div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:900,color:C.ink,...F}}>{unreadHeadline}</div><div style={{fontSize:12,color:C.stone,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",...F}}>{unreadDetail}</div></div><Ico d={IC.chev} size={18} color={C.emerald}/></div></Card></div>}
