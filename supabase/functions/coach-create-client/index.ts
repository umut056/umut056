...
async function callerProfile(req: Request) {
  ...
  const rows = await supabaseFetch(`/rest/v1/profiles?id=eq.${authUser.id}&select=id,role,status,ref_code`);
  return rows?.[0] || null;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!caller || caller.role !== "coach" || caller.status !== "active" || !caller.ref_code) {
      return json({ error: "Coach authorization required" }, 403);
    }
